 ...
 *
 * 该类用一个kEdge类构成一个邻接边的信息，包括邻接边的起始顶点与结束顶点，权值。
 * 用类kEdge创建对象，录入对象信息，按照对象的权值进行比较，符合条件的对象加入
...
public class Kruskal {
 ...
 /*
  * Max:定义顶点数组的最大值
  * kEdge:链表kEdge，存储构造的kEdge对象
  * target:链表trget,存储最终得到结果的kEdge对象
  * parent:存储顶点信息的数组
  * n:顶点数
  */
    int Max=100;
    ArrayList<kEdge> kEdge=new ArrayList<kEdge>();
    ArrayList<kEdge>target=new ArrayList<kEdge>();
    ...
    public void kruskal(){
        int i=0;                 //顶点
        while(i<n-1&&kEdge.size()>0){       //如果只有一条边或者没有边跳出
            double min=Double.MAX_VALUE;
            kEdge temp=null;
            for(int j=0;j<kEdge.size();++j){      //遍历图形
                kEdge tt=kEdge.get(j);
